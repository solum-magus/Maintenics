...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_resolved'])) {
    ...
    $sql = "UPDATE reportdetails SET status = ?, sid = ?, sname = ?, date_resolved = NOW() WHERE report_id = ?";
    $stmt = $Testsql->prepare($sql);
    $stmt->bind_param("sssi", $status, $sid, $sname, $reportId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reject_report'])) {
    ...
    $sql = "UPDATE reportdetails SET status = ?, sid = ?, sname = ? WHERE report_id = ?";
    $stmt = $Testsql->prepare($sql);
    $stmt->bind_param("sssi", $status, $sid, $sname, $reportId);
    $stmt->execute();
    ...
    <table style="width: fit-content; height: 95%; margin-right: auto; margin-block: 2%;">
        ...
        <tr>
            <td>
                ...
                <?php elseif ($report['status'] === 'Ongoing'): ?>
                    <form method="POST" action="">
                        <input type="hidden" name="report_id" value="<?= $report['report_id']; ?>">
