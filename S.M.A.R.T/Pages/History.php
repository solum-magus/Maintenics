...
$sql = "SELECT report_id, problem, date_reported, date_resolved, status, rating, feedback FROM reportdetails WHERE status = 'Resolved' ORDER BY date_reported DESC";
...
<?php endfor; ?>
                    ...
                    <?php if (empty($feedback)) { ?>
                        <textarea name="feedback" rows="4" style="width: 100%; margin-top: 10px; padding: 10px; margin-left: 130%; text-align: center;" >
                            <?= htmlspecialchars($report['feedback'] ?? '') ?></textarea>

                    <?php } else { ?>
                        <textarea name="feedback" placeholder="Leave your feedback here..." rows="4" style="width: 100%; margin-top: 10px; padding: 10px; margin-left: 130%;"></textarea>
                    <?php } ?>
